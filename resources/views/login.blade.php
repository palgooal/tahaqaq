			...
			<div class="row">
				<div class="col col-md-6 offset-md-1 hide ">
					...
					<div class="text-right ">
						...
						<h1 class="sub-heading h3 primery-color mt-5 mb-3">{{__('login.Happy to come back again!')}}</h1>
				...
				<div class="col col-md-5 col-sm-12 col-xs-12">
                    ...
                    <!-- end allert -->{{__('login.Login')}}</div>
					<form method="POST" action="/TahqqLogin">@csrf @method('POST')
						<input type="hidden" name="returnUrl" value="{{$returnUrl}}">
						<div class="form-group col col-md-12 col-sm-12 mb-3 my-4 p-0">
                        <label class="control-label fields-label">{{__('login.Email')}}</label>
							<input type="email" required="required" class="form-control input-group-sm px-5 p-4" placeholder="{{__('login.Email')}}" id="username" name="username" />@error('username')
							<p>{{$message}}</p>@enderror</div>
						<div class="form-group col col-md-12 col-sm-12  mb-3 my-4 p-0">
							<label class="control-label fields-label">{{__('login.Password')}}</label>
							<input type="password" required="required" class="form-control p-4" placeholder="{{__('login.Password')}}" id="password" name="password" />@error('password')
							<p>{{$message}}</p>@enderror</div>
						<div class="under-form-login d-flex justify-content-between align-items-center col-md- px-5">
							<div class="d-flex justify-content-center align-items-center">
								...
								<label for="defaultCheck" class="form-linkes mr-3 mb-0 ">{{__('login.Remember me')}}</label>
							</div>
							<div> <a class="form-linkes" href="https://client.tahqq.com/index.php/password/reset/begin">{{__('login.Forget Password ?')}}</a>
							</div>
						</div>
						<div class="">
							<button class="primery-button col col-md-12 col-sm-12 border-0 py-2 mt-4" type="submit" value="Login">{{__('login.Sign In')}}</button>
						</div>
					</form>

	...
	<div class="row">
		<div class="col-md-4">
            ...
            اضافة قائمة
				...
				<!--begin::Form-->
				<form method="POST" action="/pg-admin/menus" accept-charset="UTF-8" class="form-horizontal" role="form">@csrf
					<div class="card-body">
...
<path d="M12,10 C13.1045695,10 14,9.1045695 14,8 C14,6.8954305 13.1045695,6 12,6 C10.8954305,6 10,6.8954305 10,8 C10,9.1045695 10.8954305,10 12,10 Z M12,13 C9.23857625,13 7,10.7614237 7,8 C7,5.23857625 9.23857625,3 12,3 C14.7614237,3 17,5.23857625 17,8 C17,10.7614237 14.7614237,13 12,13 Z" fill="#000000" fill-rule="nonzero"></path>
						...
						<div class="collapse multi-collapse" id="multiCollapseExample1">
							<div class="card card-body">
								<div class="form-group">
									<label for="exampleInputPassword1">رابط القائمة<span class="text-danger">*</span>
									</label>
						...
						<div class="collapse multi-collapse" id="multiCollapseExample2">
							<div class="card card-body">
								<div class="form-group">
									<label for="exampleInputPassword1">رابط من الصفحات<span class="text-danger">*</span>
									</label>
									<select class="form-control" name="slug" id="">
										<option value="1" selected></option>@foreach ($pages as $page)
										<option value="/pages/{{$page->slug}}">{{$page->title_ar}}</option>@endforeach</select>
								</div>
							</div>
						</div>{{-- end toggle url --}}
						<div class="form-group">
							<label for="exampleInputPassword1">ترتيب القائمة<span class="text-danger">*</span>
							</label>
							...
							<table class="table table-head-custom table-head-bg table-borderless table-vertical-center">
								<thead>
									<tr class="text-left text-uppercase">
										<th style="min-width: 100px"><span class="text-dark-75">ترتيب القائمة</span>
										</th>
								...
								<tbody>@foreach ($menus as $menu) @include('admin.menu.editMenu')
									<tr>
										<td>{{$menu->sort}}</td>
										<td>{{$menu->name_ar}}</td>
										<td>{{$menu->url}}</td>
										<td>
											<button class="btn btn-primary" data-toggle="modal" data-target="#exampleModals{{$menu->id}}" data-whatever="@getbootstrap">تعديل</button>{{-- href="/pg-admin/menus/{{$menu->id}}/edit" --}}</td>
										<td>
											<button type="button" class="btn btn-danger" data-menuid="{{$menu->id}}" data-toggle="modal" data-target="#deletemenu">حذف</button>
										</td>@endforeach</tr>
									...
									<div class="modal fade" id="deletemenu" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												...
												<form action="{{route('menus.destroy', 'test')}}" method="post">@method('delete') @csrf
